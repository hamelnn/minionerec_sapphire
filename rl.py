...
def train(
    ...
    dapo: bool = False,
    gspo: bool = False,
    # --- additions for single-GPU / low-VRAM runs (defaults keep original behaviour) ---
    rl_sample: int = -1,   # subsample SidDataset rows for RL; -1 = use all
    do_eval: bool = True,  # set False to skip generation-heavy eval during RL
    # --- 12GB 复现方案新增 ---
    use_lora: bool = True,          # SFT 底座 + LoRA GRPO（方案 5.3）
    max_prompt_length: int = 256,   # 左截断（保留 response 前缀与近期历史）
    max_completion_length: int = 16,
    rl_seq_sample: int = 10000,     # RLSeqTitle2SidDataset 采样上限
    ...
    train_data3 = RLSeqTitle2SidDataset(train_file, category=category_dict[category], sample=rl_seq_sample)
    ...
    eval_data = SidDataset(eval_file, category=category_dict[category], sample=eval_sample)
    ...
    # Load an extra model copy only when the reward actually needs it
    # (rule/ranking rewards only need `device`; saves ~1 GB VRAM on single-GPU runs)
    ...
    training_args = GRPOConfig(output_dir=output_dir,
                                save_steps=0.1,
                                save_total_limit=2,
                                model_init_kwargs={"torch_dtype": torch.bfloat16, "use_cache": False},
    ...
    # 方案 5.3：SFT 底座 + LoRA GRPO；SID embedding 已在 SFT 学到，本阶段冻结，仅更新 LoRA
    peft_config = None
    if use_lora:
