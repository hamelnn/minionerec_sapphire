...
class ReReTrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # 组内任一候选命中（奖励>0）的组占比：衡量组级学习信号是否稀疏
        self._metrics["group_hit_ratio"].append(
            (rewards.view(-1, self.num_generations).max(dim=1).values > 0).float().mean().item()
        )
        ...
        self._metrics["nonzero_adv_ratio"].append(
            (advantages.abs() > 1e-6).float().mean().item()
        )
